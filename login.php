...
if (isset($_POST["login"])) {
    ...
    if (!empty($_POST["username"]) && !empty($_POST["password"])) {
        ...
        try {
            ...
            $stmt = $pdo->prepare('SELECT * FROM ec_UserData WHERE name = ?');
            ...
            if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                if (password_verify($password, $row['password'])) {
                    ...
                    /*
                    $sql = "SELECT * FROM ec_UserData WHERE name = $username";  //ユーザー名の有無を判定
                    $stmt = $pdo->query($sql);
                    echo $stmt;
                    foreach ($stmt as $row) {
                        $row['name'];  // ユーザー名
                    ...
                    */
                    ...
                    header("Location: order_list.php");  // メイン画面へ遷移
                    exit();  // 処理終了
                } else {
                    // 認証失敗
...
<form action="register.php">
            <input type="submit" value="ユーザー登録はコチラ">
</form>
